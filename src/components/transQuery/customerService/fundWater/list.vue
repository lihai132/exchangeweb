...
export default {
    ...
    methods: {
        ...
        // 获取表格数据
        async init() {
            this.moneyWaterType = (await request('/enum/ioFlagServiceStaff/api', this.query)).data;
			this.getData();
        },
        // 获取表格数据
        async getData() {
			...
			this.querying = true;
            var jsonData = (await request('/report/customerService/moneyWater/list', this.query)).data;
			this.querying = false;
            if(jsonData != null){
