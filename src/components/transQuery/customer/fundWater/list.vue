...
export default {
    ...
    methods: {
        ...
        // 获取表格数据
        async init() {
            this.moneyWaterType = (await request('/enum/ioFlagCustomer/api', this.query)).data;
			this.getData();
        },
        // 获取表格数据
        async getData() {
			...
			this.querying = true;
            var jsonData = (await request('/report/customer/moneyWater/list', this.query)).data;
			this.querying = false;
            if(jsonData != null){
        ...
        buttonClick(item) {
            if(item == '导出'){
                ...
                //this.query.export = 0;
                 requestExport('/report/customer/moneyWater/list', this.query).then(response => { 
