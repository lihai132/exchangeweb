<template>
    <div class="Popup">
        <el-dialog title="审核客户交易员实名认证" :visible.sync="editVisible" width="700px" :close-on-click-modal="false">
            <el-container>
                <el-main>
...
export default {
    ...
    methods: {
        ...
        async determine(opType) {
			...
			let result = await request('/account/customer/doAuthPersonCertification', this.bean);
			this.submitting=false;
			if(result.data>=0){
				this.editVisible = false;
				if(opType!='unpass'){
