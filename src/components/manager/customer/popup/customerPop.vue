                    ...
                    <div class="information">
                        <div class="news-noe">
                            ...
                            <div class="nav">
                                ...
                                <div class="line" v-if="bean.fTraderId>0&&bean.fTraderTypeChild>0">
									<div class="line-center">
       ...
       receipt(index, row) {
		...
		checkSubmit(bean){
			...
			if(commonUtils.isEmpty(bean.fName)){
				this.$message.error("客户简称不能为空");
				return false;
			}
			if(bean.fName.length>32){
				this.$message.error("客户简称不能超过32个字符");
				return false;
			}
			if(commonUtils.isEmpty(bean.fTraderTypeChild)||bean.fTraderTypeChild<=0){
			...
			if(bean.fTraderTypeChild==2){
				...
				if(isNaN(this.fBusnessReward)){
					this.$message.error("税率须是数字");
					return false;
				}
				var rate = parseFloat(this.fBusnessReward);
				if(rate<0){
					this.$message.error("税率不能小于0");
					return false;
				}
