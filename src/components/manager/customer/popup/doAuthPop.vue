            ...
            <el-container>
                <el-main>
                    <div class="information">
						...
						<div class="news-img">
						    ...
						    <div class="nav">
								...
								<div class="img">
									...
									<div slot="tip" class="el-upload__tip" align="center">身份证(正面)</div>
								</div>
								<div class="line" style="margin-left: 60px;">
									<div class="line-center" style="margin-top:50px;">
									    <div class="name required" style="display: inline-block;margin-left: 28px;">姓名：</div>
									    <el-input
									        size="small"
									        placeholder="请输入姓名"
						...
						<div class="news-img" v-if="bean.certType==2">
						    <div class="title">
...
export default {
    ...
    methods: {
         receipt(index, row) {
			...
			this.bean = popup1;
			if(this.bean.certType==1){
				this.title = "审核客户个人实名认证";
			}else if(this.bean.certType==2){
				this.title = "审核客户企业实名认证";
			}else{
				this.title = "审核客户实名认证";
			}
			this.listMembers();
        ...
        async determine(opType) {
			...
			let result = await request('/account/customer/doAuthCertification', this.bean);
			this.submitting=false;
			if(result.data>=0){
				this.editVisible = false;
				if(opType!='unpass'){
