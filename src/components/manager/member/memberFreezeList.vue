...
export default {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   this.querying = true;
		   let list = await request('/account/member/freezeList', this.query);
		   this.querying = false;
		   this.selectedRow='';
		...
		async btnClick(item,index){
			...
			if(item=='冻结'){
				...
				this.tip='您确定要对运营商['+this.selectedRow.fName+']进行冻结操作吗？';
...
            } 
        ...
        async doFreeze() {
			...
			let bean = await request('/account/member/doFreeze', {"fDealerId":this.selectedRow.fDealerId});
			this.submitting=false;
			...
			if(bean.data>0){
				...
				this.$message.error("冻结失败！可能由于页面停留时间过长，请刷新确认重试");
			}
        },
        // 分页导航
