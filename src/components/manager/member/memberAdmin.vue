    ...
    <div class="BaseTable" @click="onMouseDown">
        <div class="container">
            ...
            >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fFullName" label="管理员名称" width="220" align="center"></el-table-column>
                <el-table-column :show-overflow-tooltip="true" prop="fStatus" :formatter="statusFormat" label="账号状态" width="120" align="center"></el-table-column>
...
import popup from './popup/resetMemberAdminPwdPop.vue';
export default {
    name: 'basetable',
    data() {
        return {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   this.querying = true;
		   let list = await request('/account/memberAdmin/list', this.query);
		   this.querying = false;
		   this.selectedRow='';
		...
		async btnClick(item){
			if(item=='重置密码'||item=='解锁账号'){
				if(this.selectedRow==null||this.selectedRow==''||!(this.selectedRow.fAdminId>0)){
					this.$message.error("请选择数据");
					return;
				}
				if(this.selectedRow.fAccount=='admin'){
					this.$message.error("不允许操作超级管理员");
					return;
				}
				if(item=='解锁账号'){
					this.$confirm('确定解锁该账号吗?', '提示', {
					          confirmButtonText: '确定',
					          cancelButtonText: '取消',
					          type: 'info'
					        }).then(() => {
					          this.unlockAcount(this.selectedRow.fAdminId);
					}).catch(() => {})
					return;
				}
		...
		async unlockAcount(fAdminId){
			let result = await request('/account/admin/unlockAcount', {'fAdminId':fAdminId});
			if(result.data>0){
        ...
        getBut() {
            ...
            actionList.map(item => {
                if (item.fParentId == this.butId) {
					if(item.fName!='查询'&&item.fName!='权限查看'){
