            ...
            <el-container>
                <el-main>
                    <div class="information">
                        <div class="news-noe">
                            <div class="title">
                                <div class="left" style="width: 130px;">- 运营商信息</div>
                            </div>
                            <div class="nav">
								<div class="line">
    ...
    methods: {
       async receipt(index, row) {
            ...
            if(this.bean.showType=='add'){
            	...
            	this.titleName='修改运营商基本信息';
				fDealerId = this.bean.fDealerId;
            }
			if(this.bean.fTypeChild>0){
				this.bean.fTypeChild = this.bean.fTypeChild+'';
			}
			...
			let charge = await request('/parameter/servercharge/customsize/selectByDealerId',{"fDealerId":fDealerId});
		...
		async getNextCode(){
			let nextCode = await request('/account/member/getNextCode',{});
		...
		checkSubmit(bean){
			if(commonUtils.isEmpty(bean.fCode)){
				this.$message.error("运营商编号不能为空");
				return false;
			}
			if(!(bean.fDealerId>0)&&!(/^\d{3}$/.test(bean.fCode))){
				this.$message.error("运营商编号格式有误，须是三位数字编号");
				return false;
			}
			if(commonUtils.isEmpty(bean.fName)){
				this.$message.error("运营商简称不能为空");
				return false;
			}
			if(bean.fName.length>32){
				this.$message.error("运营商简称不能超过32个字符");
				return false;
			}
			if(commonUtils.isEmpty(bean.fTypeChild)){
				this.$message.error("运营商类型不能为空");
				return false;
			}
			if(commonUtils.isEmpty(bean.fGroupId)){
				this.$message.error("服务费方案不能为空");
				return false;
			}
			...
			if(commonUtils.isEmpty(this.selectedCitys)||this.selectedCitys.length==0){
				this.$message.error("请选择所在城市");
				return false;
			}
			let areaId;
			if(commonUtils.isEmpty(this.selectedCitys)||this.selectedCitys.length==0
				||!((areaId=this.selectedCitys[this.selectedCitys.length-1])>0)){
				this.$message.error("请选择城市");
				return false;
			}
			bean.fCity = areaId;
			if(commonUtils.isEmpty(bean.fCity)){
			...
			if(bean.showType=='add'||!(bean.fDealerId>0)){
				...
				if(commonUtils.isEmpty(this.loginPwd)){
					this.$message.error("管理员登录密码不能为空");
					return false;
				}
				if(!commonUtils.loginPwdRegTest(this.loginPwd)){
					this.$message.error("管理员登录密码须是8-16位数字、字母、符号组成");
					return false;
				}
