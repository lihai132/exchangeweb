    ...
    <div class="BaseTable" @click="onMouseDown">
        <div class="container">
            <div class="head">
                <div class="head-one">
					...
					<div class="input1">
					    ...
					    <el-input v-model="query.accountOrName"  style="width:170px"></el-input>
					</div>
					<div class="but">
            ...
            >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fName" label="司机简称" width="160" align="center"></el-table-column>
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fApplicantName" label="申请人" width="140" align="center"></el-table-column>
				<el-table-column :show-overflow-tooltip="true" prop="fApplicantMobile" label="申请人手机号" width="140" align="center"></el-table-column>
...
export default {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   this.querying = true;
		   let list = await request('/account/driver/personCertificationList', this.query);
		   this.querying = false;
		   this.selectedRow='';
		...
		async btnClick(item){
			if(item=='审核实名认证'){
				...
				bean.data.fCarType = bean.data.vehicleCarType;
				bean.data.fCarLength = bean.data.vehicleCarLength;
				bean.data.fCreditCode = bean.data.vehicleCreditCode;
				bean.data.fAddress=bean.data.vehicleAddress;
				if(!commonUtils.isEmpty(bean.data.fImageCert1)){
					bean.data.fImageCert1FullUrl = commonUtils.imgBaseUrl()+bean.data.fImageCert1;
				}
				if(!commonUtils.isEmpty(bean.data.fImageCert2)){
					bean.data.fImageCert2FullUrl = commonUtils.imgBaseUrl()+bean.data.fImageCert2;
				}
				...
				if(!commonUtils.isEmpty(bean.data.fImageCar)){
					bean.data.fImageCarFullUrl = commonUtils.imgBaseUrl()+bean.data.fImageCar;
				}
				if(!commonUtils.isEmpty(bean.data.fRoadPermitImage)){
					bean.data.fRoadPermitImageFullUrl = commonUtils.imgBaseUrl()+bean.data.fRoadPermitImage;
				}
				if(!commonUtils.isEmpty(bean.data.fDrivingPermitNext)){
					bean.data.fDrivingPermitNextFullUrl = commonUtils.imgBaseUrl()+bean.data.fDrivingPermitNext;
				}
				if(!commonUtils.isEmpty(bean.data.fDrivingPermitFace)){
					bean.data.fDrivingPermitFaceFullUrl = commonUtils.imgBaseUrl()+bean.data.fDrivingPermitFace;
				}
