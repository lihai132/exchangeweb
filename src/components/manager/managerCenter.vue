    ...
    <div class="BaseTable" @click="onMouseDown">
        <div class="container">
            ...
            >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fCode" label="运营中心编号" width="140" align="center"></el-table-column>
                <el-table-column :show-overflow-tooltip="true" prop="fName" label="运营中心简称" width="200" align="center"></el-table-column>
                <el-table-column :show-overflow-tooltip="true" prop="fStatus" :formatter="statusFormat" label="运营中心状态" width="140" align="center"></el-table-column>
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fTeminateTime" label="终止时间" width="160" align="center"></el-table-column>
...
import popup from './centerPop.vue';
import resetPwdPop from './resetCenterMoneyPwdPop.vue';
import bindPop from './bindPop.vue';
import askAuthPop from './askAuthPop.vue';
import detailPop from './centerDetailPop.vue';
...
export default {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   this.querying = true;
		   let list = await request('/account/center/list', this.query);
		   this.querying = false;
		   this.selectedRow='';
		...
		async btnClick(item,index){
			if(item=='新增'){
				this.$store.commit('setPopup1', {"showType":1});
				this.$refs.popup.receipt();
				return;
			}
			if(item=='详情'||item=='修改'||item=='重置资金密码'||item=='申请实名认证'||item=='运营商绑定'||item=='申请企业认证'){
				...
				if(item=='运营商绑定'){
					var fStatus = this.selectedRow.fStatus;
					if(fStatus!=4){
						var name = this.doStatusFormat(fStatus);
						this.$message.error("当前运营中心状态为["+name+"]，[正常]状态的才可以绑定运营商");
						return;
					}
					this.$refs.bindPop.receipt();
					return;
				}
				let bean = await request('/account/center/detail', {"fDealerId":this.selectedRow.fDealerId});
				if(item=='修改'){
        ...
        getBut() {
            ...
            actionList.map(item => {
                if (item.fParentId == this.butId) {
                    if(item.fName!='查询'&&item.fName!='运营商解绑'){
