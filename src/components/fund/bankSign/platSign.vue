    ...
    <div class="BaseTable" @click="onMouseDown">
        <div class="container">
            ...
            >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fName" :formatter="codeNameFormat" label="交易所" width="200" align="center"></el-table-column>
		...
		<el-dialog
		  ...
		  width="30%"
		  >
		  <span  style="font-size: 15px; padding-left: 10px;">确定对该交易所进行解约吗？</span>
...
import popup from './popup/platSignPop.vue';
import balancePopup from './popup/platBalancePop.vue';
...
export default {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   let list = await request('/fund/bankSign/plat/list', this.query);
		   this.listing = true;
		   this.selectedRow='';
			...
			if(item=='签约'||item=='解约'||item=='查看银行卡余额'
				...
				if(item=='签约'){
					if(this.selectedRow.fSignStatus==2){
						this.$message.error("该交易所已签约，请勿重复签约");
						return;
					}
					this.$store.commit('setPopup1', JSON.parse(JSON.stringify(this.selectedRow)));
				...
				if(item=='解约'){
					if(this.selectedRow.fSignStatus!=2){
						this.$message.error("该交易所未签约，不能进行解约操作");
						return;
					}
