    ...
    <div class="BaseTable" @click="onMouseDown">
        <div class="container">
            ...
            >
				...
				<el-table-column :show-overflow-tooltip="true" prop="fMobile" label="交易员账号" width="120" align="center"></el-table-column>
				<el-table-column :show-overflow-tooltip="true" prop="fTraderName" label="交易员简称" width="110" align="center"></el-table-column>
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fStatus" :formatter="statusFormat" label="交易员状态" width="100" align="center"></el-table-column>
		...
		<el-dialog
		  ...
		  width="30%"
		  >
		  <span  style="font-size: 15px; padding-left: 10px;">确定对该{{namePrefix}}进行解约吗？</span>
...
import popup from './popup/traderSignPop.vue';
import balancePopup from './popup/traderBalancePop.vue';
...
export default {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   let list = await request('/fund/bankSign/trader/list', this.query);
		   this.listing = false;
		   this.selectedRow='';
		...
		async btnClick(item,index){
			if(item=='签约'||item=='解约'||item=='查看银行卡余额'
				...
				this.namePrefix = '交易员';
				if(this.selectedRow.fTraderType==2){
					this.namePrefix = '司机';
				}else if(this.selectedRow.fTraderType==5){
					this.namePrefix = '经纪人';
				}
				if(item=='签约'){
					if(this.selectedRow.fSignStatus==2){
						this.$message.error("该"+this.namePrefix+"已签约，请勿重复签约");
						return;
					}
					this.$store.commit('setPopup1', JSON.parse(JSON.stringify(this.selectedRow)));
				...
				if(item=='解约'){
					if(this.selectedRow.fSignStatus!=2){
						this.$message.error("该"+this.namePrefix+"未签约，不能进行解约操作");
						return;
					}
		...
		async unSign(){
			...
			let obj = {"rId":rowBean.fId,"fDealerOrTraderId":rowBean.fTraderId};
			this.submitting=true;
			let result = await request('/fund/bankSign/trader/unSign', obj);
			this.submitting=false;
