    ...
    <div class="BaseTable" @click="onMouseDown">
        <div class="container">
            ...
            >
                ...
                <el-table-column :show-overflow-tooltip="true" prop="fCode" :formatter="codeNameFormat" label="运营中心编号/简称" width="200" align="center"></el-table-column>
                <el-table-column :show-overflow-tooltip="true" prop="fStatus" :formatter="statusFormat" label="运营中心状态" width="120" align="center"></el-table-column>
		...
		<el-dialog
		  ...
		  width="30%"
		  >
		  <span  style="font-size: 15px; padding-left: 10px;">确定对该运营中心进行解约吗？</span>
...
import popup from './popup/centerSignPop.vue';
import balancePopup from './popup/centerBalancePop.vue';
...
export default {
    ...
    methods: {
       ...
       async getData() {
		   ...
		   let list = await request('/fund/bankSign/center/list', this.query);
		   this.listing = false;
		   this.selectedRow='';
		...
		async btnClick(item,index){
			if(item=='签约'||item=='解约'||item=='查看银行卡余额'
				...
				if(item=='签约'){
					if(this.selectedRow.fStatus!=4){
						this.$message.error("该运营中心状态不是[正常]，不能进行签约操作");
						return;
					}
					if(this.selectedRow.fSignStatus==2){
						this.$message.error("该运营中心已签约，请勿重复签约");
						return;
					}
					this.$store.commit('setPopup1', JSON.parse(JSON.stringify(this.selectedRow)));
				...
				if(item=='解约'){
					if(this.selectedRow.fSignStatus!=2){
						this.$message.error("该运营中心未签约，不能进行解约操作");
						return;
					}
