...
export default {
    ...
    methods: {
        ...
        async getData() {
			...
			this.data = (await request('/businessManagement/planningPromotion/detail', this.data)).data.data;
			this.data.fStatus = this.data.fStatus.toString();
            this.data.fType = this.data.fType.toString();
        },
        async commitData(){
            
            var jsonData = await request('/businessManagement/planningPromotion/update', this.data);
        ...
        receipt(index, row) {
            ...
            this.data.fPromotionId = popup1.fPromotionId;
